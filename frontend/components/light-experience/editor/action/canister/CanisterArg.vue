...
import { checkCanisterActionArgs } from '../../../types/common/checks';
...
const length = computed(() => props.action.canister.method.arg.subitems.length);

const checks = computed<boolean[]>(() => checkCanisterActionArgs(props.action, props.values));

const onEditArg = (index: number) => {
    current.value = index;
    setCurrentEditing({
        id: props.action.id,
        type: 'arg',
...
<template>
    ...
    >
        <template v-for="(child, index) in action.canister.method.arg.subitems" :key="index">
            <div class="item">
                <span>
                    <p>{{ `${index + 1}.` }}</p>
                    <p>{{ child.type.type }}</p>
                    <i>
                        {{
                            child.type.subitems
                                ? `{${child.type.subitems.length} ${$t(
                                      'plugin.actionEditor.fields',
                                  )} }`
                                : ''
                        }}
                    </i>
                    ...
                    <b @click="onEditArg(index)">
                        {{ $t('plugin.actionEditor.edit') }}
                    </b>
                </span>
            </div>
        </template>

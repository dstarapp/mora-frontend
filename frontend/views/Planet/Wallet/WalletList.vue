<template>
    <el-skeleton animated class="record" v-if="transactionsList.length < 0">
        <template #template>
            <div class="table">
                <div class="header">
...
export default defineComponent({
    setup() {
        ...
        const getList = async () => {
            let list = await LoadTransaction(store.state.current_planet_wallet_address);
            if (list) {
